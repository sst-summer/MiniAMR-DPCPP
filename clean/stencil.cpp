...
void fpga_stencil_calc(int var, int stencil_in)
{
   ...
   for (int in = 0; in < sorted_index[num_refine + 1]; in++) {
      ...
      for (int i = 0; i <= x_block_size + 1; i++)
         for (int j = 0; j <= y_block_size + 1; j++)
            for (int k = 0; k <= z_block_size + 1; k++) {
               ...
               inputArray[k + (z_block_size + 2) * (j + (y_block_size + 2) * i)] = bp->array[var][i][j][k];
            }

      t3 = timer();
      timer_stencil_2 += t3 - t2;

      sycl::buffer<double, 3> input_buffer(inputArray, num_array);

      {
         sycl::buffer<double, 3> output_buffer(outputArray, num_array);
         ...
         for (int i = 1; i <= x_block_size; i++)
            for (int j = 1; j <= y_block_size; j++)
               for (int k = 1; k <= z_block_size; k++) {
                  if (bp->array[var][i][j][k] != outputArray[k + (z_block_size + 2) * (j + (y_block_size + 2) * i)]) {
                     printf("Buffer:%.17lf -- Block:%.17lf\n", outputArray[k + (z_block_size + 2) * (j + (y_block_size + 2) * i)], bp->array[var][i][j][k]);
                     assert(bp->array[var][i][j][k] == outputArray[k + (z_block_size + 2) * (j + (y_block_size + 2) * i)]);
                  }
               }
      }
      ...
      for (int i = 1; i <= x_block_size; i++)
         for (int j = 1; j <= y_block_size; j++)
            for (int k = 1; k <= z_block_size; k++) {
               bp->array[var][i][j][k] = outputArray[k + (z_block_size + 2) * (j + (y_block_size + 2) * i)];
            }

      t7 = timer();
      timer_stencil_6 += t7 - t6;
