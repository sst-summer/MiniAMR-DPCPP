...
void fpga_stencil_calc(int var, int stencil_in)
{
   ...
   for (int in = 0; in < sorted_index[num_refine + 1]; in++) {
      ...
      float* inputArray = new float[var_max * (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)];

      //create a buffer that comes from the fpga
      float* outputArray = new float[var_max * (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)];
      ...
      sycl::buffer<float, 1> input_buffer(inputArray, num_array);

      {
         sycl::buffer<float, 1> output_buffer(outputArray, num_array);
            ...
            for (int i = 1; i <= x_block_size; i++)
               for (int j = 1; j <= y_block_size; j++)
                  for (int k = 1; k <= z_block_size; k++) {
                     if (fabs(bp->array[var][i][j][k] - outputArray[(var * (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + (k + (z_block_size + 2) * (j + (y_block_size + 2) * i))]) > pow(0.1, validation_tol) ) {
                         
                        printf("Buffer:%.17lf -- Block:%.17lf\n", 
                               outputArray[(var * (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + (k + (z_block_size + 2) * (j + (y_block_size + 2) * i))], 
                               bp->array[var][i][j][k]);
...
{
   int i, j, k, in;
   float sb, sm, sf, work[x_block_size + 2][y_block_size + 2][z_block_size + 2];
   block* bp;
