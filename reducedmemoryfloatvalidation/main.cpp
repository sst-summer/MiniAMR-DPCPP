...
#include "param.h"
   ...
   if (!my_pe) {
      for (i = 1; i < argc; i++)
         ...
         else if (!strcmp(argv[i], "--no_validate"))
            no_validate = 1;
         else if (!strcmp(argv[i], "--validation_tol"))
            validation_tol = atof(argv[++i]);
...
void allocate(void)
{
   ...
   for (n = 0; n < max_num_blocks; n++) {
      blocks[n].number = -1;
      blocks[n].array = (float ****) ma_malloc(num_vars*sizeof(float ***),
                                                __FILE__, __LINE__);
      for (m = 0; m < num_vars; m++) {
         blocks[n].array[m] = (float ***)
                              ma_malloc((x_block_size+2)*sizeof(float **),
                                        __FILE__, __LINE__);
         for (i = 0; i < x_block_size+2; i++) {
            blocks[n].array[m][i] = (float **)
                                   ma_malloc((y_block_size+2)*sizeof(float *),
                                             __FILE__, __LINE__);
            for (j = 0; j < y_block_size+2; j++)
               blocks[n].array[m][i][j] = (float *)
                                     ma_malloc((z_block_size+2)*sizeof(float),
                                               __FILE__, __LINE__);
         }
      }
   }
