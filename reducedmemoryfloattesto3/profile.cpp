...
void profile(void)
{
   ...
   if (!my_pe) {
      ...
      if (report_perf & 4) {
         ...
         printf("\n ================== End report ===================\n");
printf("Summary: ranks %d ts %d time %lf calc %lf stencil %lf max comm %lf min red %lf refine %lf blocks/ts %lf max_blocks %d\n", num_pes, num_tsteps, average[0], average[38], timer_stencil_total, maximum[37], minimum[39], average[42], ((double) total_blocks)/((double) (num_tsteps*stages_per_ts)), global_max_b);
printf("Stencil: stencil_timer_1 %lf stencil_timer_2 %lf stencil_timer_3 %lf stencil_timer_4 %lf stencil_timer_5 %lf stencil_timer_6 %lf\n", timer_stencil_1, timer_stencil_2, timer_stencil_3, timer_stencil_4, timer_stencil_5, timer_stencil_6);
fflush(NULL);
      }
   }
}
