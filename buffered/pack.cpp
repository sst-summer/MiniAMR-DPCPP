      ...
      for (i = 1; i <= x_block_size; i++)
         for (j = 1; j <= y_block_size; j++)
            for (k = 1; k <= z_block_size; k++)
               send_buff[l++] = bp->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k)  + (z_block_size + 2) * ((j) + (y_block_size + 2) * (i)))];
}
...
void unpack_block(int n)
{
   ...
   for (v = 0; v < num_vars; v++)
      for (i = 1; i <= x_block_size; i++)
         for (j = 1; j <= y_block_size; j++)
            for (k = 1; k <= z_block_size; k++)
               bp->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k)  + (z_block_size + 2) * ((j) + (y_block_size + 2) * (i)))] = recv_buff[l++];
}
