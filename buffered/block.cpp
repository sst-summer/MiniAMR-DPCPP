                     ...
                     for (i2 = i = 1; i <= x_block_half; i++, i2+=2)
                        for (j2 = j = 1; j <= y_block_half; j++, j2+=2)
                           for (k2 = k = 1; k <= z_block_half; k++, k2+=2)
                              bp1->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k2  )  + (z_block_size + 2) * ((j2  ) + (y_block_size + 2) * (i2  )))] =
                              bp1->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k2  )  + (z_block_size + 2) * ((j2  ) + (y_block_size + 2) * (i2+1)))] =
                              bp1->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k2  )  + (z_block_size + 2) * ((j2+1) + (y_block_size + 2) * (i2  )))] =
                              bp1->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k2  )  + (z_block_size + 2) * ((j2+1) + (y_block_size + 2) * (i2+1)))] =
                              bp1->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k2+1)  + (z_block_size + 2) * ((j2  ) + (y_block_size + 2) * (i2  )))] =
                              bp1->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k2+1)  + (z_block_size + 2) * ((j2  ) + (y_block_size + 2) * (i2+1)))] =
                              bp1->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k2+1)  + (z_block_size + 2) * ((j2+1) + (y_block_size + 2) * (i2  )))] =
                              bp1->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k2+1)  + (z_block_size + 2) * ((j2+1) + (y_block_size + 2) * (i2+1)))] =
                                    bp->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k+k1)  + (z_block_size + 2) * ((j+j1) + (y_block_size + 2) * (i+i1)))]/8.0;
               }
            ...
            for (o = 0; o < 8; o++) {
               ...
               for (v = 0; v < num_vars; v++)
                  for (i2 = i = 1; i <= x_block_half; i++, i2+=2)
                     for (j2 = j = 1; j <= y_block_half; j++, j2+=2)
                        for (k2 = k = 1; k <= z_block_half; k++, k2+=2)
                           bp->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k+k1)  + (z_block_size + 2) * ((j+j1) + (y_block_size + 2) * (i+i1)))] =
                                 bp1->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k2  )  + (z_block_size + 2) * ((j2  ) + (y_block_size + 2) * (i2  )))] +
                                 bp1->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k2  )  + (z_block_size + 2) * ((j2  ) + (y_block_size + 2) * (i2+1)))] +
                                 bp1->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k2  )  + (z_block_size + 2) * ((j2+1) + (y_block_size + 2) * (i2  )))] +
                                 bp1->array[((v) *  (x_block_size + 2) * (y_block_size + 2) * (z_block_size + 2)) + ( (k2  )  + (z_block_size + 2) * ((j2+1) + (y_block_size + 2) * (i2+1)))] +
