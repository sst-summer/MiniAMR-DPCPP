...
void fpga_kernel(sycl::buffer<double, 1>& input_buffer, sycl::buffer<double, 1>& output_buffer, sycl::buffer<int, 1>& control_buffer)
{
   //Device queue submit
   queue_event = device_queue.submit([&](sycl::handler& cgh) {
      ...
      auto accessor_control = control_buffer.get_access<sycl::access::mode::read>(cgh);

      cgh.single_task<class Stencil_kernel>([=]() {

         double local_array[40][12][12][12];
         double work[40][12][12][12];

          if(accessor_control[0] == 0){
             ...
             for (int var = 0; var < 40; var++) {
                ...
                for (int i = 0; i <= 11; i++)
                   for (int j = 0; j <= 11; j++)
                      for (int k = 0; k <= 11; k++)
                         local_array[var][i][j][k] = accessor_in[(var * (12) * (12) * (12)) + (k + (12) * (j + (12) * i))];
               ...
               for (int j = 1; j <= 10; j++)
                  for (int k = 1; k <= 10; k++)
                     work[var][i][j][k] = (
                        ...
                        local_array[var][i][j - 1][k] +
                        local_array[var][i][j][k - 1] +
            ...
            for (int i = 1; i <= 10; i++)
              for (int j = 1; j <= 10; j++)
                 for (int k = 1; k <= 10; k++) {
                    ...
                    accessor_out[(var * (12) * (12) * (12)) + (k + (12) * (j + (12) * i))] = work[var][i][j][k];
                    local_array[var][i][j][k] = work[var][i][j][k];
              }
         }
         });
